...
class PITUniverseStore:
    ...
    def universe(self, signal_date) -> Tuple[pd.DataFrame, dict]:
        ...
        known_col = _col(df, KNOWN_COLS)
        if known_col:
            known = pd.to_datetime(df[known_col], errors="coerce")
            if (known.notna() & known.gt(day)).any():
                raise PITUniverseError(f"{snap.path} 存在 known_at 晚于信号日的成员。")
        canonical_col = "canonical_fund_id" if "canonical_fund_id" in df.columns else None
        if canonical_col and df[canonical_col].dropna().duplicated().any():
            raise PITUniverseError(f"{snap.path} 存在重复 canonical_fund_id。")
        ...
        if inception_col:
            ...
            if (inc.notna() & inc.gt(day)).any():
                raise PITUniverseError(f"{snap.path} 存在成立日晚于信号日的成员。")
