...
def configure_http_url(pro, http_url: str | None):
    """Apply an explicitly configured compatible Tushare endpoint."""
    if http_url:
...
def get_pro(args):
    ...
    return configure_http_url(pro, args.http_url or os.environ.get("TUSHARE_HTTP_URL"))
...
def load_requested_codes(path) -> list[str]:
    """Load a deterministic, de-duplicated Tushare ts_code allow-list."""
    if not path:
        return []
    with open(path, encoding="utf-8-sig") as handle:
        return list(dict.fromkeys(line.strip() for line in handle if line.strip()))
...
def _code_list(pro, args, extra: list[str] | None = None) -> list[str]:
    requested = load_requested_codes(args.codes_file)
    if requested:
        return requested[: args.limit] if args.limit else requested
...
def main() -> None:
    ...
    ap.add_argument("--http-url", default=None,
                    help="兼容 Tushare DataApi 的 HTTPS 端点；也可用 TUSHARE_HTTP_URL")
    ...
    ap.add_argument("--codes-file", default=None,
                    help="每行一个 ts_code；提供后逐基金接口只抓清单内代码")
